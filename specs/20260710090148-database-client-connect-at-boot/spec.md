...
| 5 | Where should the aggregated schema the shared client binds to live, given `eslint-plugin-boundaries` forbids a `shared`-context infrastructure file from depending on another context's (`prompt`'s) infrastructure — discovered when the original `globalSchema.ts` module (under `src/modules/shared/infrastructure/database/`) failed `npm run lint` during implementation? | Inside `src/config.ts`'s existing `database.schema` object, which already aggregates schemas across contexts for the legacy client and is exempt from boundaries checks (it sits outside `src/logic/*`/`src/modules/*`). The existing legacy imports are renamed `legacyPromptSchema`/`legacyUserSchema` to flag them as temporary until the `@logic/*` migration retires them; the modules-side `prompt` schema is added under a plain name and mixed into the same object: `schema: { ...legacyPromptSchema, ...legacyUserSchema, ...promptSchema }`. `config.ts` keeps a single default export — no new named export. Rejected alternatives: adding a new `"config"` boundaries element type (a bigger, project-wide rule change) and relocating to a new top-level `src/globalSchema.ts` file (works, but the user preferred consolidating into `config.ts`'s existing object). | Removes the `globalSchema.ts` module from plan §2; `config.ts` is now in scope (was previously stated as untouched); `services` (shared) binds to `config.database.schema` directly. |